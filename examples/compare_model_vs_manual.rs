...
fn main() {
    ...
    let mut indexed_actual: Vec<(usize, f32)> =
        logits_actual_data.iter().cloned().enumerate().collect();
    indexed_actual.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    let rank_actual = indexed_actual
        .iter()
    ...
    let rank_manual = indexed_manual
        .iter()
        .position(|(idx, _)| *idx == 17)
        .unwrap()
        + 1;
    ...
    println!(
        "  Token 17 rank: actual={}, manual={}",
        rank_actual, rank_manual
    );
    println!(
        "  Token 17 logit: actual={:.4}, manual={:.4}",
        logits_actual_data[17], logits_manual[17]
    );
    println!();
    ...
    let mut k_caches: Vec<Vec<Vec<Vec<f32>>>> =
        vec![vec![vec![vec![0.0; head_dim]; max_seq_len]; num_kv_heads]; 24];
    ...
    // Check rank of token 17
    let mut indexed_actual2: Vec<(usize, f32)> =
        logits_actual2_data.iter().cloned().enumerate().collect();
    indexed_actual2.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    let rank_actual2 = indexed_actual2
        .iter()
        .position(|(idx, _)| *idx == 17)
        .unwrap()
        + 1;

    let mut indexed_manual2: Vec<(usize, f32)> =
        logits_manual2.iter().cloned().enumerate().collect();
    indexed_manual2.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    let rank_manual2 = indexed_manual2
        .iter()
        .position(|(idx, _)| *idx == 17)
        .unwrap()
        + 1;
    ...
    println!(
        "  Token 17 rank: actual={}, manual={}",
        rank_actual2, rank_manual2
    );
    println!(
        "  Token 17 logit: actual={:.4}, manual={:.4}",
        logits_actual2_data[17], logits_manual2[17]
    );
