...
fn compare_vecs(name: &str, manual: &[f32], backend_out: &[f32], tolerance: f32) {
    ...
    let max_diff: f32 = manual
        .iter()
        .zip(backend_out.iter())
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);

    let mean_diff: f32 = manual
        .iter()
        .zip(backend_out.iter())
        .map(|(a, b)| (a - b).abs())
        .sum::<f32>()
    ...
    if max_diff > tolerance {
        ...
        // Find worst elements
        let mut diffs: Vec<(usize, f32, f32, f32)> = manual
            .iter()
            .zip(backend_out.iter())
            .enumerate()
...
fn main() {
    ...
    backend
        .rms_norm(&input_tensor, &attn_norm_w, eps, &mut backend_normed)
        .unwrap();
    ...
    backend
        .vec_mat_q(&normed_tensor, &wq, &mut backend_q)
        .unwrap();
    ...
    println!(
        "  Manual: min={:.4}, max={:.4}, first5={:?}",
        q_min,
        q_max,
        &manual_q[..5]
    );
    let bq_min = backend_q_data.iter().cloned().fold(f32::INFINITY, f32::min);
    let bq_max = backend_q_data
        .iter()
        .cloned()
        .fold(f32::NEG_INFINITY, f32::max);
    println!(
        "  Backend: min={:.4}, max={:.4}, first5={:?}",
        bq_min,
        bq_max,
        &backend_q_data[..5]
    );
    ...
    // Backend: apply rope
    let mut backend_q_rope =
        Tensor::from_f32(backend_q_data, vec![num_heads, 1, head_dim]).unwrap();
    let mut dummy_k = Tensor::zeros(vec![1, 1, head_dim], DType::F32);
    backend
        .rope(&mut backend_q_rope, &mut dummy_k, pos, freq_base, 1.0, true)
        .unwrap();
    ...
    if let Some(q_bias_info) = gguf.data.get_tensor("blk.0.attn_q.bias") {
        ...
        // Manual: Q + bias
        let manual_q_biased: Vec<f32> = manual_q
            .iter()
            .zip(q_bias_data.iter())
            .map(|(q, b)| q + b)
            .collect();
        ...
        let linear =
            Linear::new(wq.clone(), Some(q_bias.clone())).expect("Failed to create Linear");
        let mut backend_q_biased = Tensor::zeros(vec![num_heads * head_dim], DType::F32);
        linear
            .forward(&normed_for_linear, &mut backend_q_biased, &backend)
            .unwrap();
        ...
        compare_vecs(
            "Linear (Q + bias)",
            &manual_q_biased,
            backend_q_biased_data,
            1e-3,
        );

        println!("\n  Q with bias stats:");
        let mqb_min = manual_q_biased
            .iter()
            .cloned()
            .fold(f32::INFINITY, f32::min);
        let mqb_max = manual_q_biased
            .iter()
            .cloned()
            .fold(f32::NEG_INFINITY, f32::max);
        println!("  Manual: min={:.4}, max={:.4}", mqb_min, mqb_max);
        let bqb_min = backend_q_biased_data
            .iter()
            .cloned()
            .fold(f32::INFINITY, f32::min);
        let bqb_max = backend_q_biased_data
            .iter()
            .cloned()
            .fold(f32::NEG_INFINITY, f32::max);
