...
fn main() {
    ...
    println!(
        "Embedding stats: min={:.4}, max={:.4}, mean={:.4}, std={:.4}",
    ...
    for tok in [0, 1, 16, 17, 28, 100, 1000] {
        ...
        println!(
            "  Token {:5}: min={:+.4}, max={:+.4}, mean={:+.4}, std={:.4}, first3={:.4?}",
            tok,
            t_min,
            t_max,
            t_mean,
    ...
    let diff_16_17: f32 = emb_16
        .iter()
        .zip(emb_17.iter())
        .map(|(a, b)| (a - b).abs())
        .sum();
    let diff_16_28: f32 = emb_16
        .iter()
        .zip(emb_28.iter())
        .map(|(a, b)| (a - b).abs())
        .sum();
    let diff_17_28: f32 = emb_17
        .iter()
        .zip(emb_28.iter())
        .map(|(a, b)| (a - b).abs())
        .sum();
    ...
    println!(
        "Output weight stats: min={:.4}, max={:.4}, mean={:.4}, std={:.4}",
    ...
    for tok in [16, 17, 28, 0, 1] {
        ...
        println!(
            "  Token {:5}: min={:+.4}, max={:+.4}, mean={:+.4}, std={:.4}",
    ...
    println!();
    println!(
        "  |out_row[16] - out_row[17]| = {:.4} (should be > 0)",
        diff_rows
    );
    ...
    let test_hidden: Vec<f32> = (0..hidden_size)
        .map(|i| ((i as f32 * 0.01).sin()))
        .collect();
    let (h_min, h_max, h_mean, _) = stats(&test_hidden);
    println!(
        "Test hidden: min={:.4}, max={:.4}, mean={:.4}",
        h_min, h_max, h_mean
    );

    // Compute logit for token 17
    let logit_17: f32 = test_hidden
        .iter()
        .zip(row_17.iter())
        .map(|(h, w)| h * w)
        .sum();
    ...
    println!(
        "All logits: min={:.4}, max={:.4}, mean={:.4}, std={:.4}",
