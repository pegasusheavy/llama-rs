...
fn main() {
    ...
    println!();
    println!(
        "Embedding: [{}, {}] -> lookup token t gives row t (size {})",
        vocab_size, hidden_size, hidden_size
    );
    println!(
        "Q projection: [{}, {}] -> x @ W_q gives [{}]",
        hidden_size,
        num_heads * head_dim,
        num_heads * head_dim
    );
    println!(
        "K projection: [{}, {}] -> x @ W_k gives [{}]",
        hidden_size,
        num_kv_heads * head_dim,
        num_kv_heads * head_dim
    );
    println!(
        "V projection: [{}, {}] -> x @ W_v gives [{}]",
        hidden_size,
        num_kv_heads * head_dim,
        num_kv_heads * head_dim
    );
    println!(
        "Output proj: [{}, {}] -> attn @ W_o gives [{}]",
        num_heads * head_dim,
        hidden_size,
        hidden_size
    );
    println!(
        "FFN gate: [{}, {}] -> x @ W_gate gives [{}]",
    ...
    println!(
        "FFN down: [{}, {}] -> inter @ W_down gives [{}]",
        intermediate_size, hidden_size, hidden_size
    );
    println!(
        "LM head: [{}, {}] -> hidden @ W_lm gives [{}]",
