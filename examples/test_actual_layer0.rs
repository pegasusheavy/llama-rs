...
fn main() {
    ...
    // Manual KV cache
    let mut k_cache: Vec<Vec<Vec<f32>>> =
        vec![vec![vec![0.0; head_dim]; max_seq_len]; num_kv_heads];
    let mut v_cache: Vec<Vec<Vec<f32>>> =
        vec![vec![vec![0.0; head_dim]; max_seq_len]; num_kv_heads];
    ...
    for (pos, &tok) in tokens.iter().enumerate() {
        // Get embedding
        let h: Vec<f32> =
            emb[tok as usize * hidden_size..(tok as usize + 1) * hidden_size].to_vec();
    ...
    println!(
        "Manual Layer 0 output (pos 3): min={:.4}, max={:.4}",
        m_min, m_max
    );
    println!(
        "First 5: {:?}",
