//! GPU-only inference example
//!
//! Tests the GpuOnlyInference engine where all computation stays on GPU.

#[cfg(feature = "cuda")]
fn main() {
    use llama_rs::gguf::GgufFile;
    use llama_rs::model::ModelLoader;
    use llama_rs::tokenizer::Tokenizer;
    use llama_rs::sampling::{Sampler, SamplerConfig};
    use llama_rs::backend::cuda::gpu_only::GpuOnlyInference;
    ...
    // Create GPU-only inference engine
    eprintln!("\nCreating GPU-only inference engine...");
    let upload_start = Instant::now();
    let mut gpu = GpuOnlyInference::from_model(&model, 2048).expect("Failed to create GPU inference");
    ...
    for _ in 0..n_tokens {
        ...
        let logits_tensor = llama_rs::tensor::Tensor::from_f32(&logits, vec![logits.len()]).unwrap();
    ...
    eprintln!("Generated {} tokens in {:.2}s ({:.2} tokens/sec)", 
              generated, gen_time.as_secs_f32(), tokens_per_sec);
}
