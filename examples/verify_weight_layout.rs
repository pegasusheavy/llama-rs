...
fn main() {
    ...
    println!(
        "  Q dim: {} x {} (hidden -> num_heads*head_dim)",
        hidden_size,
        num_heads * head_dim
    );
    println!(
        "  K dim: {} x {} (hidden -> num_kv_heads*head_dim)",
        hidden_size,
        num_kv_heads * head_dim
    );
    println!(
        "  V dim: {} x {} (hidden -> num_kv_heads*head_dim)",
        hidden_size,
        num_kv_heads * head_dim
    );
    println!(
        "  O dim: {} x {} (num_heads*head_dim -> hidden)",
        num_heads * head_dim,
        hidden_size
    );
    println!();
    ...
    if let Some(info) = gguf.data.get_tensor("blk.0.attn_q.weight") {
        ...
        println!(
            "       y has shape [{}] (num_heads * head_dim = {} * {} = {})",
            dims[1],
            num_heads,
            head_dim,
            num_heads * head_dim
